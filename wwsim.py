...
# This is used to save the global variables across multiple runs
...
def load_settings(filename="settings.json"):
    try:
        ...
        return None  # or default settings
...
if settings:
    q_table = np.array(settings['q_table'])  # Convert list back to numpy array
    epsilon = settings['epsilon']
else:
    # Define default values if settings.json doesn't exist or is empty
    q_table = np.zeros((ROWS, COLUMNS, NUM_BUCKETS, NUM_ACTIONS))  # Initialize Q-table with zeros
    epsilon = 0.1
        ...
        for j in range(COLUMNS):
            for k in range(NUM_BUCKETS):
                for l in range(NUM_ACTIONS):
                    if l == 0:
                        q_table[i, j, k, l] = 0.125
                    elif l == 1:
                        q_table[i, j, k, l] = 0.375
                    elif l == 2:
                        q_table[i, j, k, l] = 0.625
                    else:
                        q_table[i, j, k, l] = 0.875
...
#
        ...
        num_episodes = 10
        episode_count = 0
        if epsilon < MAX_EPSILON: # checks if we need to run the learning
            win_count = 0
            for _ in range (num_episodes):
                sim.agent.reset()
                sim.reset_stats(0) 
                sim.generate_simulation()
                sim.agent.set_param(epsilon, LEARNING_RATE, DISCOUNT)
                sim.agent.set_table(q_table)
                while (sim.terminal_test() is not True) and (sim.endEpisode is not True ):
                    # Prompt agent to move
                    sim.move()
                    sim.update_score()
                    moveCount = moveCount + 1
                # Episode is over -- Last updates
                if (sim.endEpisode):
                    sim.agent.has_gold()
                    win_count += 1
                elif sim.agentPos == sim.wumpusLoc:
                    sim.agent.is_dead()
                elif sim.agent.give_up:
                    print('Agent gave up')
                else:
                    sim.agent.is_dead()                
                
                # Need to update table one more time when game ends 
                sim.agent.sarsa_update_terminal()
                
                # Update epsilon
                if episode_count < num_episodes:
                    # Calculate the increment per episode
                    increment = (MAX_EPSILON - sim.initial_epsilon) / num_episodes
                    # Update epsilon
                    epsilon += increment
                    # Ensure epsilon does not exceed the maximum
                    epsilon = min(epsilon, MAX_EPSILON)
                # Increment the episode counter
                episode_count += 1
                q_table = copy.deepcopy(sim.agent.q_table)
                print(q_table)
...
# Example checkpoint after modifying settings
save_settings({
    'q_table': q_table.tolist(),  # Convert numpy array to list for JSON serialization
    'epsilon': epsilon
})
